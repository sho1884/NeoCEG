...
アルゴリズムは、デシジョンテーブルが満たすべき**義務**（§1.4）を1つずつ果たしていく手続きである。

1. **義務の一覧化**: 論理式網羅（A）・結果網羅（B）・制約の実演（C）をモデル定義順に並べる
2. **列の生成**: 未達成の義務ごとに、その義務を主目的とする列を1本作る。義務の要求値と、
   その値を結果ノードまで通す感度化条件（§4.7）を同時に置く
3. **完成と整理**: 制約完了で列を完成させ、重複列を落とし、完成形でカバレッジを判定する
4. **弱テスト削除**: 担う義務がすべて他の列でも担われている列を削除する
5. **分類と報告**: 果たせなかった義務を実行不可／テスト不可／観測不能に分類する

詳細は §5.1。旧版にあった「敗者復活フェーズ」は、結果網羅を義務 B として最初から
一覧に含めることで不要になった（§5.1）。

---

### 1.4 デシジョンテーブルの正しさ（義務集合）

生成されたデシジョンテーブル `T`（列の集合）が**正しい**とは、次の3つが同時に成り立つことをいう。
...
| J | **決定性** — 同一ソースからは同一のデシジョンテーブル | 走査順に依存しない | — |
...
各列が**なぜ存在するか**を1つ記録する。生成した列と1対1に対応する。

```
origins[t] = { kind: A|B|C, 識別子, 観測対象ノード, 観測先の結果ノード }
```

| 種別 | 識別子 | 観測対象 | 観測先 | 表示形式 |
|------|--------|---------|--------|---------|
| A | 論理式番号 | オーナーノード | 感度化経路の終端 | `Expr.27 個別審査要因_あり → 主契約引受結果_個別審査` |
| B | 原因と値 | その原因 | 感度化経路の終端 | `現在の治療_あり=T → 主契約引受結果_個別審査` |
| C | MASK 制約 | （なし） | （なし） | `MASK(就業状態_非就業 → 職業危険度_通常, 職業危険度_危険)` |

**担っている義務すべてではなく、主義務1つだけを記録する。** 列が担う義務の一覧は
カバレッジ表が `#` / `x` で示しており、素性が答えるのは「なぜこの列が存在するか」の一点である。

フェーズ3（重複列の除去）で列を落とすときは、**残した列の素性を保つ**。
弱テストの素性も保持する。カバレッジ表には弱テストも残るため（§13.2）、
そこで「なぜ作られ、なぜ不要になったか」が読める。

素性を表示するのは**カバレッジ表だけ**である（§16.3）。理由はそこに記す。

### 3.8 不適切マーク `unsuitables[i]`
...
順序を定義順に固定することで、同一ソースからは同一のデシジョンテーブルが得られる（義務 J）。
...
デシジョンテーブルの生成は、義務集合（§1.4）を1つずつ果たしていく手続きである。
...
FUNCTION calcTable(model) -> AlgorithmState:
    ...
    FOR each obligation o IN obligations:
        ...
        IF column == null:
            CONTINUE                                     // フェーズ6で分類する
        tests.append(column)
        origins.append(o の素性)                          // §3.7: この列が存在する理由
...
`chooseCauseValue()` は `completeColumn()`（§6.2）から `"t"` / `"f"`（小文字）で呼び出される。
**原因への値割り当ては合流履歴 `applied[]`（§3.6）に残さない。** バックトラックのあとは、
再構築された作業配列に対して割り当てをやり直すためである。

不適切と分かった値の組は `unsuitableCauseValues` に記録し、同じ列で再試行しない。
...
1. **効果が限定的**: 敗者復活テストは元の論理式を厳密にはカバーしない
   ...
   テストとなります」と述べている。
2. **義務 B（結果網羅）が代替手段を提供**: INCL不可能な論理式の
   キー入力（例: D=T）が他のテストに出現しない場合、義務 B（§1.4）が
   T/F両方の出現を保証する列を生成する。敗者復活よりも精度は
...
将来的に実装する場合は、義務の一覧（§1.4）に第4の種別として加え、
他の義務と同じ経路で列を生成する設計が妥当と考えられる。
...
FOR each expression l:
    ...
    FOR each test t (テスト順、左から右):
        ...
        ELSE IF origins[t] がこの論理式を主義務とする:
            表示[t][l] = "@"      // この列はこの式を検証するために作られた（§3.7）
            firstCovered = TRUE
        ELSE IF weaks[t] == 1:
...
`@` は「この列の存在理由」を示すため、弱テストであっても付ける。
生成の順序上、主義務のセルはその式を最初にカバーする非弱テストであることが多いが、
重複除去や弱テスト削除の結果、別の列が先に来ることもある。その場合 `#` は
「最初にカバーしたが、そのために作られたわけではない列」を意味する。

### 13.3 シンボルの意味

| シンボル | 名称 | 意味 |
|---------|------|------|
| `@` | 主義務 | この論理式を検証するために**作られた**テスト条件（§3.7。各論理式につき最大1つ） |
| `#` | 初回カバレッジ | この論理式を**初めてカバーした**非弱テスト条件（そのために作られたのではない） |
...
カバレッジ表の**末尾**に、各列の素性（§3.7）を1行で示す。**弱テストを含む全列**が対象である。
カバレッジ表はデシジョンテーブルがその形になった根拠を説明する表であり（§13）、
削除された列についても「なぜ作られ、なぜ不要になったか」が読める必要がある。

**素性を示すのはカバレッジ表だけである。** 素性は `Expr.27` のように論理式番号を含むが、
その番号が何を要求する行なのかはカバレッジ表にしか書かれていない。デシジョンテーブルだけを配布された
読み手には解釈できないため、デシジョンテーブルには目的行を置かない。デシジョンテーブルは実行するための成果物とし、
その形になった理由の説明はカバレッジ表が担う。
...
**作成日**: 2026-02-05
**更新日**: 2026-09-02（デシジョンテーブルの正しさを義務集合として定義: §1.4。観測可能性を導入し「果たす」の定義に組み込み: §2.5。カバレッジ判定を実行されるテストの性質に変更し大文字小文字を問わないものに: §2.2 / §13.1。感度化条件を新設: §4.7。生成を義務単位のパイプラインに再構成: §5.1 / §5.2 / §6 / §7 / §12。弱テスト削除を義務 A∪B∪C の単一規則に統合し、旧フェーズ2「敗者復活」と MASK 実演保護をその帰結に: §14。観測不能状態と記号 `>` を追加: §13.4。合流履歴を `applied[]` に: §3.6）
