...
アルゴリズムは、決定表が満たすべき**義務**（§1.4）を1つずつ果たしていく手続きである。
...
カバレッジ表は決定表がその形になった根拠を説明する表であり（§13）、
削除された列についても「なぜ作られ、なぜ不要になったか」が読める必要がある。

**素性を示すのはカバレッジ表だけである。** 素性は `Expr.27` のように論理式番号を含むが、
その番号が何を要求する行なのかはカバレッジ表にしか書かれていない。決定表だけを配布された
読み手には解釈できないため、決定表には目的行を置かない。決定表は実行するための成果物とし、
