...
The returned `status` / `multiEffect` drive the **always-visible validity banner** (GUI §7.5), not just a
comment buried in the text. / 戻り値の `status` / `multiEffect` が**常時表示の妥当性バナー**（GUI §7.5）を
...
- **Always-visible validity banner (GUI §7.5).** The panel's `useMemo` computes the `SkeletonResult`; its
...
7. **Validity warnings / 妥当性の警告**: surface **both** warning A (skeleton not verified) and B
   ...
   active tab and collapse state (GUI §7.5). Advisory (amber), never blocking. / 警告 A（スケルトン未検証）
   と B（複数効果列）の**両方**を、**常時表示**のパネルバナーに出す（タブ・開閉非依存。GUI §7.5）。助言的
   （amber）でブロックしない。
